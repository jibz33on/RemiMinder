...
create table public.summaries_log (
  ...
  cost_usd numeric(10, 6) null default 0,
  created_at timestamp with time zone null default now(),
  constraint summaries_log_pkey primary key (id),
  constraint fk_transcript foreign KEY (transcript_id) references visit_transcripts (transcript_id) on delete CASCADE
) TABLESPACE pg_default;

create index IF not exists idx_summaries_log_transcript_id on public.summaries_log using btree (transcript_id) TABLESPACE pg_default;

create index IF not exists idx_summaries_log_created_at on public.summaries_log using btree (created_at desc) TABLESPACE pg_default;
